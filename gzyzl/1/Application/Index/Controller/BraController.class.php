 ...
 *
 */
class BraController extends InitCore{
    ...
    public function savePortrait(){
        ...
        if($info['fileSize'] > 0){
            session('temp_portrait_url',$info['fileUrl']);
            session('temp_portrait_angle',$angle);
            $return['error'] = '0';
    ...
    public function saveGame(){
      ...
      //  $pose_font_url = I('post.pose_font_url');   //字体 url
        ...
        //保存的用户上传图片的url
        $data['portrait_url'] = session('temp_portrait_url');
        $data['img_angle'] = session('temp_portrait_angle');
        $data['pose_girl_url'] = $pose_url;
        //$data['pose_font_url'] = $pose_font_url;
        $data['sticker_url'] = $sticker_url;
        $data['sticker_top'] = $top;
        $data['sticker_left'] = $left;
        $data['bra_type'] = $bra_type;
    ...
    public function share(){
        ...
        $pdo = new \initcore\Database('mysql');
        $pre_sql = "select * from bra where bra_id=:id";
     ...
     * 用户访问登录记录
     */
    private function _braRecord($log_out = false){
        $ip = get_client_ip();
