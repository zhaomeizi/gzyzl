...
/***/ function(module, exports, __webpack_require__) {
	...
	var methods = {
	  ...
	  'setZoom': function setZoom(newZoom) {
	    return callOnFirst(this, 'setZoom', newZoom);
	  },
	  ...
	  setImageSrc: function setImageSrc(imageSrc) {
	    return callOnFirst(this, 'setImageSrc', imageSrc);
	  },
	...
	var Cropit = (function () {
	  ...
	  _createClass(Cropit, [{
	    ...
	    value: function init() {
	      ...
	      this.preImage.onerror = function() {
		  ...
		  if(this.options.previewInImg)
		  {
			  this.$preview = this.options.$preview.css({ background: 'none', overflow: 'hidden' });
			  this.$previewImg = (0, _jquery2['default'])('<img />').addClass(_constants.CLASS_NAMES.PREVIEWIMG).attr('alt', '');
			  this.$preview.append(this.$previewImg);
	          this.$previewImg.on(_constants.EVENTS.PREVIEW_MOVE, this.onMove.bind(this));
			  this.$previewImg.css({
				  position:'absolute', 
				  display:'block',
	      ...
	      if (this.options.imageBackground) {
	        ...
	        this.$preview.hover(function () {
			  if(_this.options.previewInImg)
			  {
				 _this.$previewImg.addClass(_constants.CLASS_NAMES.PREVIEW_HOVERED);
			  }
	          _this.$imageBg.addClass(_constants.CLASS_NAMES.PREVIEW_HOVERED);
	        }, function () {
			  if(_this.options.previewInImg)
			  {
	          	_this.$previewImg.removeClass(_constants.CLASS_NAMES.PREVIEW_HOVERED);
			  }
	    ...
	    value: function bindListeners() {
	      ...
	      if(this.options.imageBackground && this.$imageBg)
	      {
	    	this.$imageBg.on(_constants.EVENTS.PREVIEW, this.onPreviewEvent.bind(this));   	  
	      }
	      if (this.options.allowDragNDrop) {
	    ...
	    value: function unbindListeners() {
	      ...
	      if(this.options.imageBackground && this.$imageBg)
	      {
	    	  this.$imageBg.off(_constants.EVENTS.PREVIEW);
	      }
	    ...
	    value: function onPreImageLoaded() {
	      ...
	      //设置了image的crossOrigin属性后，在iPhone微信上this.image加载图片会失败
	    ...
	    value: function onImageLoaded() {
		  ...
		  if(this.options.previewInImg)
		  {
		  	  this.$previewImg.attr("src",this.imageSrc);
		  }
		  else
		  {
	    ...
	    value: function onPreviewEvent(e) {
	      ...
	      if(this.options.imageBackground && this.$imageBg)
	      {
	    	  this.$imageBg.off(_constants.EVENTS.PREVIEW_MOVE);
	      }
	      if (e.type === 'mousedown' || e.type === 'touchstart') {
	        this.origin = this.getEventPosition(e);
	        if (e.originalEvent && e.originalEvent.touches && e.originalEvent.touches.length > 1) {
	        	this.distancePx = this.getDistanceBetweenTouches(e);
	        }
	        this.moveContinue = true;
	        this.$preview.on(_constants.EVENTS.PREVIEW_MOVE, this.onMove.bind(this));
	        if(this.options.imageBackground && this.$imageBg)
		    {
		    	this.$imageBg.on(_constants.EVENTS.PREVIEW_MOVE, this.onMove.bind(this));
		    }
	      } else {
	    ...
	    value: function onMove(e) {
	      ...
	      if (this.moveContinue && eventPosition && this.origin) {
	        this.setOffset({
	          x: this.offset.x + eventPosition.x - this.origin.x,
	          y: this.offset.y + eventPosition.y - this.origin.y
	        });
	      	if (e.originalEvent && e.originalEvent.touches && e.originalEvent.touches.length > 1) {
				var newDisPx = this.getDistanceBetweenTouches(e);
				var scalePx = newDisPx - this.distancePx;
				this.distancePx = newDisPx;
				var imageBgSize = Math.sqrt( Math.pow(this.$imageBg.width(),2) + Math.pow(this.$imageBg.height(),2) );
				var newZoom = this.zoom + scalePx * 2 / $(window).width();//imageBgSize;
	    ...
	    value: function setOffset(position) {
		  ...
		  {
			  this.$previewImg.css({
				  left: this.offset.x - window.parseInt(this.$preview.css('border-left-width') || 0),
				  top: this.offset.y - window.parseInt(this.$preview.css('border-top-width') || 0)
			  });
		  }
		  else
		  {
	    ...
	    value: function setZoom(newZoom) {
		  ...
		  if(this.options.previewInImg)
		  {
			  this.$previewImg.css({
				  width: updatedWidth - 
				  		window.parseInt(this.$preview.css('border-left-width') || 0) - 
				  		window.parseInt(this.$preview.css('border-right-width') || 0),
				  height: updatedHeight - 
			  		window.parseInt(this.$preview.css('border-top-width') || 0) - 
			  		window.parseInt(this.$preview.css('border-bottom-width') || 0)
			  });
		  }
		  else
		  {
	    ...
	    value: function getCroppedImageData(exportOptions) {
	      ...
	      var dataUrl = "";
	      try {
	    	  dataUrl = canvas.toDataURL(exportOptions.type, exportOptions.quality);
	      } catch(e) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var options = {
	  ...
	  values: [{
	    ...
	    name: 'previewInImg',
	    type: 'boolean',
	    description: 'Whether or not to preview the image in a img target.',
	    'default': false
	  }, {
		...
		name: 'distancePx',
		type: 'number',
		description: 'Distance between touches',
		'default': 0
	  }, {
	    name: 'freeMove',
	    type: 'boolean',
